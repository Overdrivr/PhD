...
It relies on equivalent-time sampling for performing time-domain measurement with a much larger bandwidth that real-time oscilloscopes.
...
On the other hand, a time-equivalent sampler measures waveforms point by point, and require N repetitions of the waveform to acquire the \textit{N} samples.
...
The main disavantage is that it requires as many measurements as amount of points needed.
...
Measurement structures are directly integrated into silicon, and using a few synchronisation signals are used to record waveforms of noise inside the susbtrate.
The architecture of this sensor is a simple latch comparator \ref{fig:noise-detect-latch-comparator}.
