...
The gun is composed of a metallic discharge tip to inject the discharge.
...
Its shape can impact a lot test results and failure levels.
Historically, ESD gun were famous for lacking some reproducibility on test results \cite{hmm-uncertainty}, for all reasons described previously.
...
The pulse starts by a fast peak with a $1ns$ risetime approximately.
It is followed by a slower slope of smaller amplitude but longer duration.
Voltage levels can reach 15kV peak with 30A of current.
...
The RC network alone though does not suffice to reproduce the waveform. given in .
Parasitic devices play an important part in shaping the waveform.
Fig. \ref{fig:esd-gun-model} provides a physically-based ESD Gun model (TODO: Ref) that helps understand the impact of parasitic devices.
...
% Talk about HMM
IEC 61000-4-2 \cite{iec61000-4-2} and ISO 10605 \cite{iso10605} are system-level standards, targeting equipment testing and qualification.
The \gls{hmm} \cite{hmm} specification aims to adapt those system-level standards for integrated circuits testing.
