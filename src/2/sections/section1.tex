...
ESD protection designate a class of devices rather than a particular device.
They can be implemented in multiple ways, with diodes, thryistors, \gls{tvs}, silicon-control rectifiers, etc.
...
They are modelled in very various ways, overall a distinction is made between physically-based models and behavioral models.

% Physical modelling
Physical modelling is often referred as TCAD modelling, for Technology Computer-Aided Design.
...
TCAD devices can be represented as 2D slice or 3D device \ref{fig:esd-protection-tcad-model}.
In a TCAD model, device model is built by describing the manufacturing process
Types of manufacturing step (lithography, diffusion, etc)
Shape of masks
Then the TCAD program simulates the device manufacturing.
The model is represented by a mesh, composed of triangle primitives.
After this step, simulator knows for each triangle the concentration of materials at the area occupied by the triangle.
2D slices are faster to simulate but fail to reproduce corner effects, where the rectangular shape of a corner creates higher electric fields than the sides, and usually results in a weaker device than expected by 2D.

\begin{figure}[!h]
  \centering
  \includegraphics[width=0.3\textwidth]{src/2/figures/esd_protection_tcad_model.png}
  \caption{2D and 3D mesh representations of ESD protections for TCAD simulation}
  \label{fig:esd-protection-tcad-model}
\end{figure}

TCAD model is a possible first step to build a behavioral model of the protection.
Among other parameters, it enables to extract the I(V) response of the ESD protection.
Alternative is to use the ESD protection measurement I(V)

% Behavioral modelling
Behavioral modelling is more widely used for studying the interaction of the protection with a circuit.
Relies on the I(V) curve extracted from the TCAD or a measurement to build an equivalent model
It is possible to couple TCAD and SPICE simulations (TODO: ref), but it is faster to simulate a behavioral model than a physically based model.
Multiple modelling methods exist, such as behavioral and compact models \cite{modelling-diode-esd, esd-diode-compact-model, tvs-modeling}.
...
In the case of snapback devices, the impedance change is very abrupt, leading to discontinuities of the slope of the I(V) response.
...
With classic \gls{spice} simulations, those events are particularly difficult to model.
Multiple issues rise, convergence, accuracy
To overcome those difficulties, modelling languages like VHDL-AMS or Verilog-A are extremely helpful
They enable to describe models with very useful primitives such as equations formulation, direct support for signal integration and derivative, combined digital and analog solvers for improved convergence, .
Using VHDL-AMS for modelling ESD protections was proposed by N. Monnereau in \cite{}.
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.3\textwidth]{src/2/figures/iv_state_machine.png}
  \caption{ESD protection state machine and corresponding I(V) curve}
  \label{fig:esd-protection-state-machine}
\end{figure}

A possible implementation in VHDL-AMS is given in Listing \ref{}.
Active machine state is described with a digital variable Y.
Initially, it is at the value X.
When voltage crosses Vt and becomes superior, Y takes the value of xxx.
At this point, the state machine has switched state, and the protection changes of slope.
For the protection to return to the off state, the condition is that the current through the protection returns back to a very small value.
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.3\textwidth]{src/2/figures/cmc_model.png}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.3\textwidth]{src/2/figures/ferrite_bead.png}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.3\textwidth]{src/2/figures/ferrite_bead_model.pdf}
