...
Bascically, this black box model is intented to act as a replacement for the \gls{ic} function during \gls{esd} board-level simulations.
...
It was shown that modelling of active outputs directly with a \gls{tlp} characterization and a bias does not seem to work and more research is required on this topic.
...
The piecewise-linear modelling technique, usually employed in the ESD field for modelling ESD protections, should be improved by adding more fitting points to fit more closely the extracted I(V) curve.
