...
However, the Verilog-A I(V) curve model is just a voltage controlled resistor and is not capable of regulating a voltage.
A voltage source is required in the model to reproduce this behavior.
...
The testbench is identical to the input validation testbench given in Fig. \ref{fig:compare-veriloga-model-input}, except this time the output waveform is being recorded.
...
Capacitor is pre-charged at 2V and not 2.5V to see if the model is capable of charging at 2.5V.

The DC value of the model is wrong, and is always increasing.
This can be explained by looking at the I(V) curve of the output magnified between 0V and 5V (see Fig. \ref{fig:tlp-output-cz-zoomed}).
At the beginning of the simulation, the I(V) model sees a potential difference of 2.5V accross its terminals.
For a 2.5V voltage, the I(V) curve model produces a value of 0.75 mA.
This constant current will keep on charging the capacitor, and
This means that even though the output voltage at the beginning of the simulation is correct, it will increase because of the Verilog-A I(V) model.
The combination of the DC source and the I(V) curve does not seem suitable for modelling an active output.

% TODO: Was it actually good in the other way for connections ?
...
Configuring the model to generate a zero current at 2.5 V will not solve the problem either because with a resistive load the output will need to supply a DC current at 2.5V.
A more complex solution must be conceived and developped to solve those issues.
The raw I(V) curve extracted from TLP does not seem to be usable as-is to model an active output.
This analysis has enable to highlights the current issues to solve before being able to model active outputs for ESD simulations.
Key milestone for black box modelling.
...
%TODO: Talk about Simulation AAS as an alternative to black box models ?
% Opening work
