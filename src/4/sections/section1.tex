...
Basically, pins were chosen because they play a key role on the regulation function.
%TODO DETAIL THE PINS PER BLOCK, MAKE A TABLE
...
% Which characterization threshold
...
% Simulation process
%TODO: DETAIL MORE SIMULATION PROCESS
% PARAMETERIZED SIMULATION WITH MULTIPLE AMPLITUDE/LENGTH

The characterization curves are given in Figs. \ref{pre_regu_wb}, \ref{bandgap_wb} and XXX.
...
After the characterization phase, it is now possible to chain the models together to evaluate the entire function's robustness.
...
The result will then be compared by performing a complete simulation of the entire regulation function (all three block).
This simulation uses transistor-level models, and will serve as a reference to compare the models against.
The same rectangular pulse will be injected on the global input, and the global output will be monitored as well.

% Show the waveform on the same pulse config

% Compare both
Afterwards, the reference simulation and the model-predicted failure level on the global output are compared.
This way, the validity of the model can be checked against at least a complete transistor-level simulation.

% What's the result

% Results for many different pulse configs
%TODO: MAKE A TABLE THAT COMPARES, FOR A BUNCH AND AMPLITUDE AND LENGTH, DIFFERENCES BETWEEN MODEL AND FULL TRANSISTOR SIM
